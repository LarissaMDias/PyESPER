def process_uncertainties(
    param, default_factor, MeasUncerts, PredictorMeasurements, n
):
    """
...
def measurement_uncertainty_defaults(
    ...
    temp_defu = np.tile(
        (
            ...
            or "temperature" in PredictorMeasurements
            else 0
        ),
        n,
    )
    ...
    for param, factor in parameters.items():
        Uncertainties_pre[param], DUncertainties_pre[param] = (
    ...
    Uncertainties_pre.update(
        dict(
            zip(
                meas_uncerts_keys, [sal_u, temp_u, *Uncertainties_pre.values()]
            )
        )
    )
    DUncertainties_pre.update(
        dict(
            zip(
                meas_uncerts_keys,
                [sal_defu, temp_defu, *DUncertainties_pre.values()],
            )
        )
    )
